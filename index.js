...
process.stdin.on('end', function () {
   var pallets = ['A', 'B', 'C'],
      ...
      weight = 1000
   ;
   var result = knapsack_logistics(pallets, width, weight, packages);
   pallets.map(function (palletName) {
      var packingString = result.pallets[palletName].reduce(function (str, items, index) {
         str += index === 0 ? items[0] : ',' + items[0];
         return str;
      }, '');
      console.log(palletName + ':' + packingString);
   });
});
